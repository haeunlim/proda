...
function Academy(props){
    ...
    for(let i=0; i<props.topics.length; i++){
        ...
        tabs.push(  <Link key={t.id} to={`/academy/`+t.link} className={`${location.pathname.includes('/academy/'+ t.link) ? 'active' : ''}`}>{t.title}</Link>)
    }
...
const Tab = ({code}) => {
    ...
    return (
        <>
        {code = "academy" ? <Academy topics={academyTp}/> : ""}
